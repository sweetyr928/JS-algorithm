function solution(fees, records) {
    ...
    while(newR.length){
        let time = Number(newR[0][0].split(":")[0]) * 60 
        + Number(newR[0][0].split(":")[1])
        if(newR[0][2] === "IN") {
          temp.push(newR[0][1])
    ...
    for(let key in car){
      let total = car[key].reduce((a,c) => a+=c,0) > fees[0] ? 
          Math.ceil((car[key].reduce((a,c) => a+=c,0) - fees[0])/fees[2]) * fees[3] : 0
